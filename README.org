...
#+begin_src sh
chmod +x NVIDIA-Linux-x86_64-595.58.03
#+end_src

Install the Nvidia driver

#+begin_src sh
sudo ./NVIDIA-Linux-x86_64-595.58.03 --install-compat32-libs --no-nvidia-modprobe --no-backup --no-kernel-module --no-x-check --no-nouveau-check --no-cc-version-check --no-kernel-module-source --no-check-for-alternate-installs --install-libglvnd --skip-depmod --no-systemd
#+end_src
...
#+begin_src sh
sudo ./NVIDIA-Linux-x86_64-595.58.03 --uninstall
#+end_src
