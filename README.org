...
[[https://www.blackmagicdesign.com/uk/products/davinciresolve][DaVinci Resolve]] installed in a Rocky Linux 10 Jail on Freebsd
...
🎥 [[https://youtu.be/GMizsbwZNJE][Davinci Resolve on Freebsd using a Rocky Linux Jail with Cuda]]
...
GPU: Discrete with at least 2GB VRAM, supporting CUDA 13.2. 
#+end_quote
...
#+begin_src sh
fetch 'https://dl.rockylinux.org/pub/rocky/10/images/x86_64/Rocky-10-Container-Base.latest.x86_64.tar.xz'
#+end_src
...
#+begin_src sh
doas tar xvfp Rocky-10-Container-Base.latest.x86_64.tar.xz -C /usr/local/jails/linux/rocky
#+end_src
...
Enable the CodeReady Linux Builder (CRB) repository. The CRB repository contains extra libraries and developer tools. This repository is available on Rocky Linux 10 but is disabled by default.
...
#+begin_src sh
dnf groupupdate multimedia --setopt="install_weak_deps=False" --exclude=PackageKit-gstreamer-plugin
#+end_src
...
copy the uvm_ioctl_override.c from this repository to the jail

#+begin_src sh
cp davinci-resolve-freebsd-jail-rocky/rocky/home/username/.config/gpu/uvm_ioctl_override.c /usr/local/jails/linux/rocky/home/"${USER}"/.config/gpu
#+end_src

uvm_ioctl_override.c code
...
we now need to build the dummy-uvm.so

inside the rocky jail change directory into the gpu directory

#+begin_src sh
cd /home/"${USER}"/.config/gpu/
#+end_src

build the dummy-uvm.so inside the rocky jail
...
https://www.nvidia.com/download/driverResults.aspx/265870/en-us/

download the driver with fetch

#+begin_src sh
fetch 'https://us.download.nvidia.com/XFree86/Linux-x86_64/595.58.03/NVIDIA-Linux-x86_64-595.58.03.run'
#+end_src
...
#+begin_src sh
sudo ./NVIDIA-Linux-x86_64-595.58.03.run --install-compat32-libs --no-nvidia-modprobe --no-backup --no-kernel-module --no-x-check --no-nouveau-check --no-cc-version-check --no-kernel-module-source --no-check-for-alternate-installs --install-libglvnd --skip-depmod --no-systemd
#+end_src
...
#+begin_src sh
sudo ./NVIDIA-Linux-x86_64-595.58.03.run --uninstall
#+end_src
...
#+begin_src sh
sudo ./NVIDIA-Linux-x86_64-VERSION.run --install-compat32-libs --no-nvidia-modprobe --no-backup --no-kernel-module --no-x-check --no-nouveau-check --no-cc-version-check --no-kernel-module-source --no-check-for-alternate-installs --install-libglvnd --skip-depmod --no-systemd
#+end_src
...
move the zip file into the rocky linux jail and unzip it
...
#+begin_src sh
sudo ./DaVinci_Resolve_20.3.2_Linux.run --appimage-extract
#+end_src
